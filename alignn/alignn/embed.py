...
def get_predictions(
    ...
    # ingest test set
    test_set = pd.read_csv(args.test_set_file, header=None).rename({0: 'file_path'}, axis=1)
    atoms = []
    ...
    with torch.no_grad():
        ...
        for dat, _id in zip(test_loader, ids):
            g, lg, target = dat
            embedding = model.embed([g.to(device), lg.to(device)])
            embedding = embedding.cpu().numpy()
            embeddings.append(embedding)

    with open(args.test_set_save_file, 'wb') as f:
        pickle.dump({'embeddings': embeddings, 'm_id': pd.DataFrame(mem)['jid']}, f)
